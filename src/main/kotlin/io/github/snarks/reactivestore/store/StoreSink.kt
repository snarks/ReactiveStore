...
interface StoreSink<in K : Any, in V : Any> {
	fun update(key: K, updater: Updater<V>)
}

fun <K : Any, V : Any> StoreSink<K, V>.cacheSinkFor(key: K): CacheSink<V> {
	val orig = this
...
fun <K : Any, V : Any, L : Any> StoreSink<K, V>.mapKeys(fn: (L) -> K): StoreSink<L, V> {
	val orig = this
...
fun <K : Any, V : Any> StoreSink<K, V>.clear(key: K) {
	update(key, Updater.clear())
}

fun <K : Any, V : Any> StoreSink<K, V>.put(key: K, newValue: V) {
	update(key, Updater.set(newValue))
}

fun <K : Any, V : Keyed<K>> StoreSink<K, V>.put(newValue: V) {
	put(newValue.key, newValue)
}

fun <K : Any, V : Any> StoreSink<K, V>.fail(key: K, error: Throwable) {
	update(key, Updater.fail(error))
}

fun <K : Any, V : Any> StoreSink<K, V>.load(
		key: K,
...
fun <K : Any, V : Any> StoreSink<K, V>.reload(
		key: K,
		customLoader: Loader<V>? = null,
		ignoreIfUpdated: Boolean = true) {
...
fun <K : Any, V : Any> StoreSink<K, V>.revert(key: K) {
